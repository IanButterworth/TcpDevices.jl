...
measurement).
"""
get_bandwidth(i::Instr{PicoVNA106}) = i_query(i, "BWFACT?")
...
"""
function set_bandwidth(i::Instr{PicoVNA106}, n)
...
"""
function get_volt_ac(ia::Instr{PicoVNA106})
    write(ia, "POWE?")
...
"""
set_volt_ac(i::Instr{PicoVNA106}, n::Voltage) = write(i, "POWE $(raw(n))"*"V")


"""
    get_impedance(Instr{PicoVNA106})
...
"""
function get_impedance(ia::Instr{PicoVNA106})
...
function perform_single_acquisition(ia::Instr{PicoVNA106})
    write(ia, "HOLD")
    write(ia, "TRGS INT")
    write(ia, "SING")
    return nothing
...
function get_acquisition_status(ia::Instr{PicoVNA106})
    write(ia, "*OPC?")
    output = read(ia)
...
"""
    set_measurement_to_complex(ia::Instr{PicoVNA106})
...
"""
function set_measurement_to_complex(ia::Instr{PicoVNA106})
    write(ia, "MEAS COMP")
    return nothing
end
...
"""
    set_measurement_to_impedance_and_phase(ia::Instr{PicoVNA106})
Set Traces A & B to measure |Z| & θ, respectively
|Z|: Impedance amplitude (absolute value)
  θ: Impedance phase
"""
function set_measurement_to_impedance_and_phase(ia::Instr{PicoVNA106})
    write(ia, "MEAS IMPH")
    return nothing
end
...
"""
get_channel(i::Instr{PicoVNA106}) = query(i, "TRAC?") == "A" ? 1 : 2

"""
    set_channel(i::Instr{PicoVNA106}, n::Int)
Sets which channel the impedance analyser is using. `n` must be 1 or 2.
"""
function set_channel(i::Instr{PicoVNA106}, n::Int)
    !(n in [1,2]) && error("Channel cannot be: $n (must be 1 or 2)")
    n == 1 ? write(i, "TRAC A") :  write(i, "TRAC B")
end


function read_float32(ia::Instr{PicoVNA106})
...
function set_data_output_to_float32(ia::Instr{PicoVNA106})
    write(ia, "FORM2")
    return nothing
end


function request_data_trace(ia::Instr{PicoVNA106})
    write(ia, "OUTPDTRC?")
    return nothing
end


function get_data_header(ia::Instr{PicoVNA106})
...
function read_num_bytes(ia::Instr{PicoVNA106}, num_bytes)
